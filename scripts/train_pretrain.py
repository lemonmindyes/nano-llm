...
def save_checkpoint(model, optimizer, step, config, vocab_size, save_path, ddp, master):
    if ddp:
...
def main():
    ...
    train_dataset = ClimbMixDataset(
        '/root/autodl-tmp/climbmix-400b-shuffle/train',
        buffer_size=5
    )
    train_loader, train_sampler = climb_mix_dataloader(
        train_dataset,
        tokenizer=tokenizer,
    ...
    while step < total_steps:
        ...
        for _, (x, y, loss_mask) in enumerate(train_loader):
            ...
            if step % save_steps == 0:
                save_checkpoint(
                    model, optimizer, step, config, vocab_size, save_path, ddp, master
                )
                if ddp:
    ...
    if step > start_step and step % save_steps != 0:
        save_checkpoint(model, optimizer, step, config, vocab_size, save_path, ddp, master)
        if ddp:
            dist.barrier(device_ids=[local_rank])
